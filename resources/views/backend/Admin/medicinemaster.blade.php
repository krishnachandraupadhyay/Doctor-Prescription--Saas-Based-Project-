...
@section('title', 'Medicine Category Formulary — Admin Console')
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
            <div>
                <h4 class="fw-bold text-dark mb-1">
                    <i class="bi bi-diagram-3-fill me-2 text-primary"></i>Medicine Formulary &amp; Category Master
                </h4>
                <p class="text-muted fs-13 mb-0">
                    <span class="text-primary fw-medium">Clinical Master</span> &bull; Manage categories, dosages, intervals, durations, units &amp; pharma companies
                </p>
            </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                    <div class="card-header bg-white py-3 px-4 d-flex flex-wrap align-items-center justify-content-between gap-3 border-bottom">
                        <h5 class="card-title mb-0 fw-bold text-dark fs-15">
                            <i class="bi bi-table me-2 text-primary"></i>Medicine Categories List
                        </h5>

                        <div class="d-flex align-items-center gap-2">
                            <div class="position-relative" style="min-width: 240px;">
                                <input type="search" id="search" class="form-control form-control-sm pe-3 rounded-pill bg-light border" placeholder="Search category..." style="padding-left: 36px;">
                    ...
                    <!-- Pagination -->
                    @if(method_exists($categor, 'hasPages') && $categor->hasPages())
                    <div class="card-footer bg-white py-3 px-4 border-top d-flex flex-wrap align-items-center justify-content-between gap-3">
                        <div class="text-muted fs-12">
                            Showing <strong>{{ $categor->firstItem() }}</strong> to <strong>{{ $categor->lastItem() }}</strong> of <strong>{{ $categor->total() }}</strong> categories
                        </div>
                        <div>
                            {{ $categor->links('pagination::bootstrap-5') }}
                        </div>
                    </div>
                    @endif
                </div>
